...
final class NotificationManager: NSObject, NotificationManagerProtocol {
    ...
    final func updateTabBarBadge(badgeTotalCount: Int) {
        Task { @MainActor in
            ...
            do {
                try await UNUserNotificationCenter.current().setBadgeCount(badgeTotalCount)
            }
            catch {
                DDLogError("[NotificationManager] Failed to set badge count to \(badgeTotalCount) with error: \(error)")
            }
        }
    }
