...
final class SafeManager: NSObject, SafeManagerProtocol {
    ...
    private static var backupIsRunning = false {
        didSet {
            let name = NSNotification.Name.backupInProgressStatus
            NotificationCenter.default.post(name: name, object: nil, userInfo: [name.rawValue: backupIsRunning])
        }
    }
    ...
    private var safeAPIService: SafeApiServiceProtocol
    ...
    init(
        ...
        safeStore: SafeStore,
        safeAPIService: SafeApiServiceProtocol,
    ...
    @available(*, deprecated, message: "testServer(server:user:password:) async throws -> (Int, Int))")
    ...
    func testServer(
        ...
        let data = try await safeAPIService.testServer(server: serverURL, user: user, password: password)
        guard let config = SafeJsonParser().getSafeServerConfig(from: data) else {
            throw SafeApiService.SafeApiError.requestFailed(message: "Invalid response data")
        }
        return (config.maxBackupBytes, config.retentionDays)
    }
