...
public final class MessageProvider: NSObject {
        ...
        do {
            ...
            context =
                if MessageProvider.configuration.backgroundFetching {
                    try DatabaseContext.directBackgroundContext(
                        with: persistenceManager.databaseManager.persistentStoreCoordinator
                    )
                }
                else {
                    persistenceManager.databaseManager.databaseContext().main
                }
        }
        catch {
            fatalError("\(error)")
    ...
    public init(
        ...
        context: ThreemaManagedObjectContext
    ) {
    ...
    deinit {
        DatabaseContext.removeDirectBackgroundContext(self.fetchedResultsController.managedObjectContext)
    }
    ...
    private func configureChangeObservation(for context: NSManagedObjectContext) {
        ...
        NotificationCenter.default.addObserver(
            ...
            name: DatabaseContext.batchDeletedAllConversationMessages,
            object: nil
        )
    ...
    public func loadMessages(around date: Date) -> Guarantee<NSManagedObjectID?> {
        Guarantee { seal in
            messageFetcher.numberOfMessages(after: date) { numberOfMessagesAfterDate in
                ...
                // If all messages at the bottom are loaded we don't need to set any threshold to load more at the
                // bottom
    ...
    @objc private func batchDeletedAllMessagesInAConversation(notification: Notification) {
        guard let userInfo = notification.userInfo as? [String: Any],
              let conversationObjectIDOfMessageDeletion =
              userInfo[DatabaseContext.conversationObjectIDKey] as? NSManagedObjectID
        else {
            return
        }
...
extension MessageProvider: NSFetchedResultsControllerDelegate {
    ...
    private func convert(
        ...
        if let groupEntity = messageFetcher.group(
            for: conversationObjectID,
            using: fetchedResultsController.managedObjectContext
        ) {
            isInactiveGroup = groupEntity.state.intValue != 0
        }
        ...
        for reloadedItemIdentifier in snapshot.reloadedItemIdentifiers {
            ...
            //
            // If the chat is an inactive group, all cells are reloaded to prevent a crash due to the reconfiguration of
            // the cells if the conversation is deleted.
            func reloadOrReconfigureItems(for identifier: NSManagedObjectID) {
                if messageWithDeletedMessageObjectIDs.contains(identifier) || isInactiveGroup {
